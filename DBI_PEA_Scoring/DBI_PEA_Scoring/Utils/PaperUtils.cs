...
namespace DBI_PEA_Scoring.Utils
{
    public class PaperUtils
    {
        ...
        internal static Dictionary<string, string> DMLType(Candidate candidate, string studentId, string answer, int questionOrder)
        {
            ...
            {
                foreach (var candidateOfQSchema in Constant.PaperSet.QuestionSet.QuestionList.ElementAt(0).Candidates)
                {
                    queryDbForDml = string.Concat(queryDbForDml, candidateOfQSchema.Solution, "\n");
                }
            }
